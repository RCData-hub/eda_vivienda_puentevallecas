...
from bokeh.models import (
    HoverTool, Legend, ColumnDataSource, 
    Select, CheckboxGroup, RangeSlider, Div,
    LinearAxis, Range1d, CustomJS, Button
)
from bokeh.palettes import Category20, Set3, Viridis256, Turbo256
...
def load_csv_data(file_path, encoding='utf-8'):
    ...
    try:
        ...
        for enc in encodings:
            for sep in separators:
                try:
                    ...
                    expected_columns = ['distrito', 'year', 'price_m2_total', 'price_m2_nuevas', 'price_m2_usadas']
        ...
        required_columns = ['distrito', 'year', 'price_m2_total', 'price_m2_nuevas', 'price_m2_usadas']
        missing_columns = [col for col in required_columns if col not in df.columns]
        
        if missing_columns:
            print(f"⚠️  Columnas faltantes: {missing_columns}")
            print("❌ El DataFrame debe tener las columnas: distrito, year, price_m2_total, price_m2_nuevas, price_m2_usadas")
            ...
            if column_mapping:
                ...
                raise ValueError("Columnas requeridas no encontradas y no se pueden mapear automáticamente")
        ...
        print(f"   - Registros con precio total > 0: {(df['price_m2_total'] > 0).sum()}")
...
def suggest_column_mapping(existing_columns, required_columns):
    ...
    column_keywords = {
        ...
        'price_m2_total': ['price_m2_total', 'precio_total', 'precio_m2', 'price_total', 'total'],
        'price_m2_nuevas': ['price_m2_nuevas', 'precio_nuevas', 'nuevas', 'new', 'nueva'],
        'price_m2_usadas': ['price_m2_usadas', 'precio_usadas', 'usadas', 'used', 'segunda']
    }
    ...
    for required_col in required_columns:
        ...
        for existing_col in existing_columns:
            ...
            ratio = difflib.SequenceMatcher(None, required_col.lower(), existing_col.lower()).ratio()
...
def load_sample_data():
    ...
    distritos = [
        'Puente de Vallecas', 'Entrevías', 'San Diego', 'Palomeras Bajas',
        'Palomeras Sureste', 'Portazgo', 'Numancia', 'Villa de Vallecas',
        'Casco Histórico de Vallecas', 'Santa Catalina'
    ]
    ...
    base_prices = {
        'Puente de Vallecas': 3200, 'Entrevías': 2800, 'San Diego': 2900,
        'Palomeras Bajas': 2700, 'Palomeras Sureste': 2600, 'Portazgo': 2850,
        'Numancia': 3100, 'Villa de Vallecas': 2950, 'Casco Histórico de Vallecas': 3000,
        'Santa Catalina': 2750
    }
    ...
    for distrito in distritos:
        ...
        for year in years:
            ...
            price_nuevas = price_total * np.random.uniform(1.05, 1.15)  # 5-15% más caras
            price_usadas = price_total * np.random.uniform(0.95, 1.05)  # Ligeramente más baratas
...
def create_district_evolution_plot(df):
    ...
    colors = Category20[len(distritos)] if len(distritos) <= 20 else Turbo256[::len(Turbo256)//len(distritos)]
    ...
    for i, distrito in enumerate(distritos):
        ...
        if len(distrito_data) > 0:
            ...
            # Línea principal
            p.line('year', 'price', source=source,
            ...
            # Puntos para hover
            p.circle('year', 'price', source=source,
                    size=5, 
                    color=colors[i % len(colors)], 
                    alpha=0.6)
...
def create_nuevas_vs_usadas_plot(df, selected_districts=None):
    ...
    if selected_districts is None:
        ...
        district_completeness = df.groupby('distrito')[['price_m2_nuevas', 'price_m2_usadas']].count().min(axis=1)
    ...
    for i, distrito in enumerate(selected_districts):
        ...
        if len(nuevas_data) > 0:
            p.line(nuevas_data['year'], nuevas_data['price_m2_nuevas'],
        ...
        if len(usadas_data) > 0:
            p.line(usadas_data['year'], usadas_data['price_m2_usadas'],
...
def create_interactive_dashboard(df):
    ...
    for i, distrito in enumerate(distritos):
        ...
        if len(distrito_data) > 0:
            ...
            line = p.line('year', 'price_total', source=source,
                         legend_label=distrito,
                         line_width=2,
                         color=colors[i % len(colors)],
                         visible=visible)
            
            circles = p.circle('year', 'price_total', source=source,
                              size=4, 
                              color=colors[i % len(colors)],
                              visible=visible,
                              alpha=0.6)
...
def create_statistical_analysis(df):
    ...
    yearly_stats = df.groupby('year')['price_m2_total'].agg([
    ...
    yearly_stats.columns = ['count', 'mean', 'median', 'std', 'q25', 'q75', 'min', 'max']
    ...
    # Línea de mediana
    p.line(years, yearly_stats['median'],
           line_width=3,
           color='red',
           legend_label='Mediana')
    
    # Línea de media
    p.line(years, yearly_stats['mean'],
           line_width=2,
           line_dash='dashed',
           color='green',
           legend_label='Media')
    ...
    # Puntos min/max
    p.circle(years, yearly_stats['max'],
             size=6, color='orange', alpha=0.7,
             legend_label='Máximo')
    
    p.circle(years, yearly_stats['min'],
             size=6, color='purple', alpha=0.7,
             legend_label='Mínimo')
...
def create_complete_analysis(df):
    ...
    print(f"Distrito más caro (promedio): {df_clean.groupby('distrito')['price_m2_total'].mean().idxmax()}")
    print(f"Distrito más barato (promedio): {df_clean.groupby('distrito')['price_m2_total'].mean().idxmin()}")
...
def main():
    ...
    try:
        ...
        print("🌐 Abre el archivo HTML en tu navegador para ver los gráficos interactivos")
